...
| S5-T2 | `packages/integrations-core/README.md` + additive note in root `README.md` — `integrations-api` slug evolution, `mediums-api` remains valid alias, medium-registry = presentation domain. **No** loa-freeside / `.well-known` mutation. | reviewer-visible; local-only (FR-14). |
